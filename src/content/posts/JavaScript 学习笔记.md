...
- `push(item)`：末尾添加元素，返回新数组长度
- `pop()`：删除末尾元素，返回被删除的元素
- `unshift(item)`：开头添加元素，返回新数组长度
- `shift()`：删除开头元素，返回被删除的元素
- `splice(start, deleteCount, ...items)`：删除/替换/插入元素，返回被删元素数组
- `sort((a,b)=>a-b)`：排序（默认按字符串排序），返回排序后的原数组
- `reverse()`：反转顺序，返回反转后的原数组

**2. 转换类（返回新数组，不改原数组）**

- `map(cb)`：按规则映射每个元素，返回长度相等的新数组
- `filter(cb)`：筛选符合条件的元素，返回新数组
- `slice(start, end)`：截取区间 `[start, end)`，返回新数组
- `concat(arr2)`：合并数组，返回新数组（可用 `...` 展开替代）

**3. 查找与判断类（返回元素/索引/布尔值）**

- `find(cb)`：返回第一个符合条件的元素，找不到返回 `undefined`
- `findIndex(cb)`：返回第一个符合条件的索引，找不到返回 `-1`
- `some(cb)`：判断是否至少有一个符合条件，返回布尔值
- `every(cb)`：判断是否所有都符合条件，返回布尔值
- `includes(val)`：判断是否包含某原始值（不适用对象数组），返回布尔值
- `indexOf(val)`：查找原始值位置，返回索引或 `-1`

**4. 迭代、归约与工具类（多场景辅助）**

- `forEach(cb)`：遍历执行副作用（打印/发请求），返回 `undefined`，不支持 `await` 顺序
- `reduce((acc,cur)=>..., init)`：归纳为单个值（求和/分组/计数），返回累加结果
- `join(separator)`：将数组拼接为字符串，如 `['a','b'].join(',')` → `"a,b"`
- `Array.isArray(val)`：静态方法，精准判断是否为数组（修复 `typeof []` 的缺陷）
- `Array.from(iterable)`：静态方法，将类数组或 NodeList 转为真数组以使用 `map`/`filter`
